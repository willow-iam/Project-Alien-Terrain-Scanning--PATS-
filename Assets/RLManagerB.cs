...
public class RLManagerB : MonoBehaviour {
    ...
    public static RLManagerB Generator;
    ...
    void Start () {
        ...
        LevelTypesThisRun[0]=FirstLevel;
    ...
    //TODO: Add failsafes so that at least one of each upgrade spawns
    public List<UpgradeOption> GenerateUpgrades(Monster[] monsters){
        ...
        while(sPosses.Count>0||pPosses.Count>0||hPosses.Count>0||rPosses.Count>0){
            ...
            do{
                for(int i=0;i<UpgradeLength;i++)
                    requiredAmounts[i]=Random.value*4+1;
                amount=
                    stat==0?
                        Mathf.CeilToInt(Random.value*MenuAndGlobals.proportions.x*requiredAmounts.Sum()/5f)
                        :
                    stat==1?
                        Mathf.CeilToInt(Random.value*MenuAndGlobals.proportions.y*requiredAmounts.Sum()/5f)
                        :
                    stat==2?
                        Mathf.CeilToInt(Random.value*MenuAndGlobals.proportions.z*requiredAmounts.Sum()/5f)
                        :
                    //stat=3
                        requiredAmounts[0]<2?
                            10
    ...
    public IEnumerator GenerateFloor(){
        ...
        foreach(Background i in tileList){
            yield return StartCoroutine(NewSprite(i.GetComponent<SpriteRenderer>().sprite.texture.width,i.GetComponent<SpriteRenderer>().sprite.texture.height,3,3,(call) => {colors=call;},baseColor:color,itersTilNextYield:10000,itersPerYield:500));
